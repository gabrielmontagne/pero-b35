...
export function pout<T>(): MonoTypeOperatorFunction<T> {
  return (source$) =>
    source$.pipe(tap((content) => process.stdout.write(forceString(content))))
}
...
export function createInputTextFiles$(files: string[]) {
  ...
  return combineLatest(files.map((file) => createInputText$(file))).pipe(
    flog('BEFORE MAP'),
...
export function createInputText$(file?: string) {
  ...
  }).pipe(reduce((acc, content) => acc + content, ''))
}
