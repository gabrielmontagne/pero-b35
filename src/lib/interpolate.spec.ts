...
describe('interpolate', () => {
  ...
  it('should handle audio file interpolation with openai format', async () => {
    const result = await interpolate('[audio[test-audio.wav]]', { audioFormat: 'openai' })
    expect(result).toHaveLength(1)
    expect(result[0].type).toBe('text')
    const textPart = result[0] as ChatCompletionContentPartText
    expect(textPart.text).toMatch(/^<AUDIO_FILE path="test-audio\.wav" metadata="[^"]+"><\/AUDIO_FILE>$/)
    
    // Decode and check metadata
    const metadataMatch = textPart.text.match(/metadata="([^"]+)"/)
    expect(metadataMatch).toBeTruthy()
    
    const decodedMetadata = JSON.parse(Buffer.from(metadataMatch![1], 'base64').toString())
  ...
  it('should handle audio file interpolation with gemini format', async () => {
    const result = await interpolate('[audio[test-audio.wav]]', { audioFormat: 'gemini' })
    expect(result).toHaveLength(1)
  ...
  it('should handle mixed content with text and audio', async () => {
    const result = await interpolate('Here is some audio: [audio[test-audio.wav]] and more text')
