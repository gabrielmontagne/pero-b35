= Pero-B35

image::pero.png[Pero from Metropolis]

A lightweight CLI for comfortable command-line chat with LLMs. Named after Pero, the robot detective from Rintaro's _Metropolis_. Thin wrapper around OpenAI-compatible APIs with tool calling, multimodal input, and MCP support.

== Commands

=== `pero chat`

Process a text buffer as a conversation. The key idea: you work on a text buffer (a file, a Vim buffer, a TiddlyWiki tiddler) and pipe it through `pero chat`. The output includes the full conversation plus the new assistant response, so you can keep building on it over time.

Simplest usage - just ask a question:

```
What is 2+2?
```

The response comes back with role markers, ready for follow-up:

```
What is 2+2?

A>>
2+2 equals 4.
```

Add `Q>>` for your next question, pipe again, repeat:

```
What is 2+2?

A>>
2+2 equals 4.

Q>>
And 3+3?
```

Role markers: `Q>>` user, `A>>` assistant, `S>>` system (optional, for custom instructions).

```sh
# from stdin
cat conversation.txt | pero chat

# from file
pero chat -f conversation.txt

# with tools enabled
pero chat --default-tools -f task.txt

# output only (just the assistant response, no conversation)
pero chat -o -f conversation.txt
```

=== `pero complete`

Raw text completion. No chat, no roles, no template — the model just continues your buffer. This is for *base* models: pretrained weights with no post-training, which behave like a pure text predictor rather than an assistant.

```sh
# continue a buffer
pero complete -f draft.txt

# from stdin, unhinged
echo -n "The following is an excerpt from a lost 1974 manual on" | pero complete -T 1.2

# just the continuation, don't echo the prompt back
pero complete -o -f draft.txt

# stop at a marker
pero complete -f draft.txt --stop $'\n\n' --max-tokens 200
```

By default the prompt is echoed back followed by the continuation, so you can pipe the same buffer through repeatedly and watch it grow. `-o` gives you the continuation alone.

Lines starting with `%%%` are stripped before sending, same as `pero chat`. `__START__` / `__END__` markers are honoured — only the region between them is completed.

Options: `-m/--model`, `-g/--gateway`, `-p/--preamble`, `-o/--output-only`, `--max-tokens` (default 512), `-T/--temperature` (default 1), `--top-p`, `--frequency-penalty`, `--presence-penalty`, `--stop`, `-n`, `--seed`.

Only gateways that expose `/v1/completions` are offered: `featherless` (default), `openrouter`, `openai`, `deepseek`, `ollama`.

NOTE: as of early 2026 the large hosted base models are mostly gone — Hyperbolic decommissioned its Llama-3.1-405B-Base endpoint and OpenRouter carries no base models at all. Featherless is the remaining serverless host with real base weights: `mistralai/Mistral-Small-24B-Base-2501` (the default), `Qwen/Qwen3-14B-Base`, `marin-community/marin-8b-base`, `ibm-granite/granite-7b-base`.

=== `pero serve`

HTTP server exposing the same functionality. Useful for editor integrations or other tools.

```sh
pero serve                     # localhost:3535
pero serve -H 0.0.0.0 -P 8080  # custom host/port
```

POST plain text to `/`, get plain text back. Query params override defaults:

```sh
curl -X POST http://localhost:3535/?model=gpt-4o \
  -H "Content-Type: text/plain" \
  -d "Q>>\nHello"
```

== Configuration

Create a `.env` file in the project root with API keys for your chosen gateway(s).

=== Supported Gateways

[cols="1,2,2", options="header"]
|===
| Gateway | Environment Variable | Notes

| `openrouter` (default)
| `OPENROUTER_API_KEY`
| Access to 100+ models via https://openrouter.ai[OpenRouter]

| `openai`
| `OPENAI_API_KEY`
| Direct OpenAI API access

| `anthropic`
| `ANTHROPIC_API_KEY`
| Claude models via https://anthropic.com[Anthropic]

| `gemini`
| `GEMINI_API_KEY`
| Google's Gemini models (uses OpenAI-compatible endpoint)

| `deepseek`
| `DEEPSEEK_API_KEY`
| https://deepseek.com[DeepSeek] models

| `ollama`
| _(none)_
| Local models via http://127.0.0.1:11434[Ollama] (no API key needed)

| `moonshot`
| `MOONSHOT_API_KEY`
| Kimi models via https://platform.moonshot.cn[Moonshot AI]

| `featherless`
| `FEATHERLESS_API_KEY`
| https://featherless.ai[Featherless] - serverless HuggingFace weights, including *base* (non-instruct) models. Used by `pero complete`.
|===

Example `.env` file:

```
OPENROUTER_API_KEY=sk-or-v1-...
OPENAI_API_KEY=sk-...
ANTHROPIC_API_KEY=sk-ant-...
GEMINI_API_KEY=AI...
DEEPSEEK_API_KEY=sk-...
MOONSHOT_API_KEY=sk-...
FEATHERLESS_API_KEY=rc_...
```

Use `-g <gateway>` to select a gateway:

```sh
pero chat -g anthropic -m claude-sonnet-4-20250514
pero chat -g gemini -m gemini-2.0-flash
pero chat -g ollama -m llama3
pero chat -g openrouter -m anthropic/claude-sonnet-4.5  # default
```

=== `chat` Options

```
pero chat

Options:
      --help                  Show help                                [boolean]
      --version               Show version number                      [boolean]
  -f, --file                  file to read from                         [string]
  -m, --model                 model to use                              [string]
  -g, --gateway               gateway provider                          [string]
  -t, --tools                 tools config file(s)                       [array]
      --default-tools         include auto-discovered tools           [boolean]
  -p, --preamble              offline files to prepend to prompt         [array]
  -o, --output-only           output only, no chat formatting         [boolean]
  -r, --include-reasoning     include reasoning tokens if available   [boolean]
  -R, --reasoning-effort      low|medium|high (reasoning models)        [string]
      --include-tool          none|call|result (include @@.tools block) [string]
      --tools-placement       top|bottom (placement of @@.tools block)  [string]
      --max-tokens            maximum tokens to generate                [number]
```

=== Tools configuration

Tools are disabled by default. Enable them with:

- `--default-tools` - Load auto-discovered tools from:
  1) ./tools.yaml in current working directory
  2) ./tools.yml in current working directory  
  3) bundled default tools.yaml
- `-t, --tools <file>` - Load specific tool configuration file(s)

Combine both flags to load auto-discovered and custom tools together.

==== Bash tools

Define shell commands as tools in YAML:

```yaml
read_file:
  description: Read the contents of a file
  parameters:
    file_path: The path to the file to read
  command: cat "{{file_path}}"

search_web:
  description: Search the web
  parameters:
    query: The search query
  command: ddgr -C -x --np -n 25 "{{query}}"
  timeout: 30000  # optional: 30 second timeout

long_running_task:
  description: A task that takes a while
  parameters:
    input: Task input
  command: ./slow-script.sh "{{input}}"
  timeout: 300000  # optional: 5 minute timeout
```

Tool configuration options:

- `description`: Tool description for the AI
- `parameters`: Parameter definitions with descriptions
- `command`: Shell command template with `{{parameter}}` placeholders
- `stdin_param`: Optional parameter to pass via stdin
- `enabled`: Optional boolean to disable a tool (default: true)
- `timeout`: Optional timeout in milliseconds (default: no timeout)
  - On timeout: Tool returns error message, process is killed

==== MCP servers

Connect to https://modelcontextprotocol.io[Model Context Protocol] servers by adding `_mcp_servers` to your tools.yaml:

```yaml
_mcp_servers:
  time:
    command: uvx
    args: [mcp-server-time, --local-timezone=Europe/Warsaw]
  
  filesystem:
    command: npx
    args: [-y, "@modelcontextprotocol/server-filesystem", "/home/user/docs"]
  
  deep-research:
    command: deep-research-gemini
    args: [--report-dir, './transient']
    timeout: 1200000  # 20 minutes for long-running research
```

MCP servers are connected on startup, their tools are discovered automatically and merged with any bash tools. Environment variables are inherited from the parent process.

MCP server configuration options:

- `command`: Executable to run
- `args`: Command line arguments (array)
- `enabled`: Optional boolean to disable the server (default: true)
- `timeout`: Optional timeout in milliseconds for tool calls (default: 300000 / 5 minutes)
  - On timeout: Request is aborted, tool returns error message

=== Content interpolation

User messages support special tags for including external content:

==== Text files

```
[txt[path/to/file.txt]]
```

Includes the file content wrapped in `<FILE path="...">` tags.

==== Images

```
[img[path/to/image.jpg]]
[img[https://example.com/image.png]]
```

Local images are base64-encoded; remote URLs are passed directly.

==== Audio

```
[audio[path/to/recording.wav]]
```

Supported formats: `.wav`, `.mp3`, `.m4a`, `.ogg`, `.flac`, `.aac`, `.webm`

Audio handling varies by gateway (OpenAI uses `input_audio`, Gemini uses `inline_data`).
